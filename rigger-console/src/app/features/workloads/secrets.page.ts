  ...
  template: `
    <ng-container *transloco="let t">
      ...
      @if (confirming(); as name) {
        <r-dialog size="sm" ariaLabel="{{ t('secrets.deleteConfirm', { name }) }}" (closed)="confirming.set(null)">
      ...
      @if (creating()) {
        <r-dialog size="lg" labelledBy="secret-create-title" (closed)="closeCreate()">
          <h2 id="secret-create-title" class="text-base font-semibold">{{ t('secrets.createTitle') }}</h2>
